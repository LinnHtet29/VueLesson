  ...
  <div id="add-blog">
      ...
      <form action="">
        ...
        <div id="checkboxes">
          ...
          <button @click="post" id="post-btn">Edit Blog</button>
      </form>
      <div id="preview">
        <h3>Preview Blog</h3>
...
export default {
  ...
  methods: {
    post() {
      axios.put('/edit/' + this.id, this.blog)
        .then(response => {
  ...
  created() {
    axios.get(`/${this.id}`)
      .then((response) => {
        let fetchBlog = response.data.result.blog;
        this.blog.title = fetchBlog.title;
        this.blog.content = fetchBlog.content;
        this.blog.categories = fetchBlog.categories;
        this.blog.author = fetchBlog.author;
